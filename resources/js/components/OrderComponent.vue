                ...
                <div class="order_item" v-for="(order,index) in orders">
                    ...
                    <div class="order_item_inner">
                        <span data-title="Task" class="order_text"
                              ...
                              v-tooltip="{ content: 'Click and copy' }"
                        >
                            {{order.ip+':'+order.port+'@'+order.domain+'\\'+order.login+';'+order.password}}
                        </span>
                        <span data-title="Weight" class="order_weight">
                            {{order.weight}}
                        </span>
                        <span class="order_show">
                            <i :class="[order.show ? 'fa-angle-up' : 'fa-angle-down']"
                               class="fa " @click="showForm(order,index)" aria-hidden="true"></i>
                        </span>
                    </div>
                    <transition name="slide">
                        <div class="order_form" v-show="order.show">
                            <div class="order_form_textarea">
                                <textarea @input="getText(order,index,$event)"
    ...
    export default {
        ...
        methods:{
            ...
            showForm(order,index){
                if(typeof this.orders[index].show=="undefined"||!this.orders[index].show){
